...
@section('page_title')
<h1>添加管理员<span class="middle-sep mg-x-10"></span><a href="{{url('/admin/permission/user')}}" class="btn btn-info">返回列表</a>
</h1>
@stop
@section('content')
<div class="row">
    <form action="{{url('/admin/permission/user-create')}}" method="post" accept-charset="utf-8" class="form-horizontal">
        <div class="form-group">
            ...
            <div class="col-sm-5">
                <input type="text" name="username" class="form-control" value="{{Input::old('username')}}" placeholder="管理员登录名称">
            </div>
        </div>
        <div class="form-group">
            ...
            <div class="col-sm-5">
                <select class="selectboxit auto-submit" data-target=".filter-submit-button" name="group_id">
                    ...
                    <option value='{{$group->id}}' @if($group->id == Input::old('group_id')) selected @endif>{{$group->name}}</option>
                    @endforeach
                </select>
            </div>
        </div>
        ...
        <div class="form-group">
            ...
            <div class="col-sm-5">
                <input type="password" name="rePassword" class="form-control" value="" placeholder="再次输入密码">
            </div>
        </div>
        <hr class="clearfix">
        <div class="form-group">
