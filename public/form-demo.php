        ...
        <form class="form-signup" method="post" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"])?>">
            <div class="col-sm-6">
                <div class="form-group">
                    ...
                    <input class="form-control field" type="text" name="username" placeholder="用户名: " required autofocus />
                </div>
                <div class="form-group">
                    <label class="control-label sr-only">密码: </label>
                    <input class="form-control field" type="text" name="password" placeholder="密码: " required autofocus />
                </div>
                <div class="form-group">
                    <label class="control-label sr-only">确认密码: </label>
                    <input class="form-control field" type="text" name="password2" placeholder="确认密码: " required autofocus />
                </div>
                <div class="form-group">
                    <label class="control-label sr-only">邮箱: </label>
                    <input class="form-control field" type="text" name="email" placeholder="邮箱: " required autofocus />
                </div>
 ...
 */
define("OK", 0);
define("USERNAME_INVALID", 100);
define("PASSWORD_INVALID", 200);
define("PASSWORD_DIFF", 201);
define("EMAIL_INVALID", 300);
$messageBox = array(
    "OK" => "OK",
...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    $password = inputFilter($_POST["passowrd"]);
...
function inputChecker($username, $password, $password2, $email) {
    ...
    } else if (!preg_match("/^[\w\d]{8,32}$/", $password)) {
...
function alertError($errorCode) {
    $errorMessage = $messageBox[$errorCode];
    echo "left:";
    foreach ($messageBox as $key => $value) {
        echo "a<br/>";
        echo $key." => ".$value."<br/>";
    }
    echo $messageBox["$errorCode"];
    echo ":right";
    echo "<script> alert($errorMessage + $errorCode); </script>";
}
